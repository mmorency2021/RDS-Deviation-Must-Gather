#!/usr/bin/env python3
"""
Regenerate vDU-tagged DU (RAN site) RDS / cluster-compare HTML report.

Same layout as the CU detailed dashboard:
  ...
  - OpenShift version from must-gather ClusterVersion under DU/extracted/
...
# Same HTML with an explicit vDU filename for sharing / downloads
OUT_HTML_DU = BASE / "vDU-RDS-compliance-dashboard.html"
...
def render_cluster_compare_markdown(
    *,
    ...
    lines.append(f"- **Reference bundle:** {md_cell(ref['minor'])} telco RAN DU")
    ...
    lines.append(
        f"- **Product docs (RAN ref DU):** [OpenShift {ref['minor']}]({ref['docs_ran_du']})\n"
    )
...
def main() -> None:
    ...
    # --- Correlation rows (all + mismatch-only) and diff panels split by presence of unified diff ---
    corr_rows: list[str] = []
...
<title>vDU · RDS compliance — cluster-compare (RAN site)</title>
<meta name="description" content="vDU (RAN site) cluster-compare report: missing CRs, mismatch diffs, OMC validation, telco RDS reference."/>
...
.brand-du .brand {{ margin-bottom: 2px; }}
.brand-du .brand-sub {{ margin-bottom: 0; }}
.toc-heading {{
...
h2#bars {{ border-bottom-color: rgba(56, 189, 248, 0.5); }}
h2#bars .sec-dot {{ background: var(--accent); color: var(--accent); }}
...
h2#validation {{ border-bottom-color: rgba(167, 139, 250, 0.45); }}
h2#validation .sec-dot {{ background: var(--purple); color: var(--purple); }}
h2#correlations {{ border-bottom-color: rgba(52, 211, 153, 0.45); }}
h2#correlations .sec-dot {{ background: var(--ok); color: var(--ok); }}
h2#diffs {{ border-bottom-color: rgba(244, 114, 182, 0.45); }}
h2#diffs .sec-dot {{ background: var(--pink); color: var(--pink); }}
...
<body class="vdu-dashboard">
<div class="layout">
<aside id="table-of-contents" aria-label="vDU report contents">
  <div class="brand-du">
    <span class="site-tag" title="virtual Distributed Unit — RAN site">vDU</span>
    <div>
      <div class="brand">RDS compliance</div>
      <p class="brand-sub">RAN site · cluster-compare · telco reference</p>
    </div>
  </div>
...
<main>
  ...
  <div class="hero-h1-wrap">
    <span class="site-tag lg" id="vdu-site-tag" title="virtual Distributed Unit — RAN site">vDU</span>
  ...
  <p class="lead">
    ...
    Compare uses the <strong>telco RAN DU</strong> bundle (<code>kube-compare-reference</code>, OpenShift {esc(ref["minor"])}).
  ...
  <div class="version-line">
    ...
    · <strong>Reference bundle:</strong> {esc(ref["minor"])} telco RAN DU
  </div>
  ...
  <p class="section-lead">
    This <strong>vDU</strong> report uses the same <strong>telco RAN distributed unit (DU)</strong> kube-compare bundle as the reference design (openshift-kni/telco-reference).
  ...
  <dl class="ref-dl">
    ...
    <dt>Product docs (RAN ref DU)</dt>
    <dd><a href="{esc(ref["docs_ran_du"])}" target="_blank" rel="noopener">Telco RAN reference DU components — OpenShift {esc(ref["minor"])}</a></dd>
  ...
  <p class="section-lead"><strong>Missing CRs</strong> — reference expects these resources; they are absent or not validated. Required vs optional tiers; links open telco RAN DU documentation where available.</p>
  <h3 id="validation"><span class="sec-dot" aria-hidden="true"></span> Missing CR detail (full inventory)</h3>
  {''.join(val_sections)}
  </section>

  <footer>
    <strong>Refresh this report:</strong> run <code>python3 build-du-dashboard.py</code> from this folder after updating <code>cluster-compare-report.json</code>.
    Outputs: <code>cluster-compare-dashboard-detailed.html</code> and <code>vDU-RDS-compliance-dashboard.html</code> (same content, vDU-tagged name).
    Extract must-gather before compare (example): <code>tar -xf &lt;must-gather-archive&gt;.tar</code> into <code>extracted/</code>.
  </footer>
</main>
</div>
<script>
...
"""
    ...
    md_doc = render_cluster_compare_markdown(
        document_title="vDU (RAN site) — RDS compliance (cluster-compare)",
        intro=(
            ...
            "**Missing configurations**. This vDU-site must-gather is compared to the **telco RAN DU** reference "
            f"(`kube-compare-reference`, OpenShift {ref['minor']})."
        ),
        must_gather_label=mg_label,
        build_script="python3 build-du-dashboard.py",
        extract_note="Extract must-gather before compare (e.g. `tar -xf <archive>.tar` into `extracted/`).",
        cv=cv,
        s=s,
        ref=ref,
        val=val,
    ...
    OUT_HTML_DU.write_text(doc)
    print("Wrote", OUT_HTML, "bytes", len(doc))
    print("Wrote", OUT_HTML_DU, "bytes", len(doc))
